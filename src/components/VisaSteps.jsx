...
export default function VisaSteps({
  ...
  const [stages, setStages] = useState(() => Array.isArray(client?.stages_data) ? client.stages_data : []);
  ...
  const saveAsCountryTemplate = async () => {
    try {
      const { data: existingTemplate } = await supabase
        .from('visa_templates')
        .select('id')
        .eq('destination', client.destination)
        .maybeSingle();

      if (existingTemplate) {
        await supabase.from('visa_templates').update({ stages_data: stages }).eq('id', existingTemplate.id);
      } else {
        await supabase.from('visa_templates').insert([{ destination: client.destination, stages_data: stages }]);
      }
      alert(`تم حفظ الهيكل كقالب رسمي لدولة (${client.destination}) بنجاح!`);
    } catch (err) {
      console.error(err);
  ...
  // حساب حالة الخطوة بناءً على البنود
  ...
  // حساب حالة المرحلة بناءً على الخطوات
  const getStageStatus = (stage) => {
    if (!stage.steps || stage.steps.length === 0) return 'pending';
    const statusCounts = {
      completed: stage.steps.filter(s => getStepStatus(s) === 'completed').length,
      in_progress: stage.steps.filter(s => getStepStatus(s) === 'in_progress').length,
      pending: stage.steps.filter(s => getStepStatus(s) === 'pending').length,
    };
    
    if (statusCounts.completed === stage.steps.length) return 'completed';
    if (statusCounts.completed > 0 || statusCounts.in_progress > 0) return 'in_progress';
    return 'pending';
  };
  ...
  const getStatusLabel = (status) => {
    switch(status) {
      ...
      case 'in_progress': return '⚙️ قيد العمل';
  ...
  const updateText = (stageIndex, stepIndex, itemIndex, field, value) => {
    const newStages = [...stages];
    if (itemIndex === null && stepIndex === null) newStages[stageIndex].title = value;
    else if (itemIndex === null) newStages[stageIndex].steps[stepIndex].title = value;
    else newStages[stageIndex].steps[stepIndex].items[itemIndex][field] = value;
    setStages(newStages);
  };
  ...
  const getOverallStatus = () => {
    ...
    const statusCounts = {
      ...
      in_progress: stages.filter(s => getStageStatus(s) === 'in_progress').length,
      pending: stages.filter(s => getStageStatus(s) === 'pending').length,
    };
    
    if (statusCounts.completed === stages.length) return 'completed';
    if (statusCounts.completed > 0 || statusCounts.in_progress > 0) return 'in_progress';
    return 'pending';
  };
  ...
  return (
    <div className="max-w-[95%] mx-auto mt-6 px-4 pb-32" dir="rtl">
      {/* رأس ثابت */}
      <div className="flex justify-between items-center bg-white p-4 rounded-xl shadow-sm border border-slate-200 mb-6 sticky top-4 z-40">
        <div className="flex items-center gap-4">
          <div className="bg-slate-100 px-4 py-2 rounded-lg font-bold text-slate-700">العميل: {client?.first_name}</div>
          <div className="bg-slate-100 px-4 py-2 rounded-lg font-bold text-slate-700">البلد: {client?.destination}</div>
          <div className={`px-4 py-2 rounded-lg font-bold border-2 ${getStatusColor(getOverallStatus())}`}>
            حالة الملف: {getStatusLabel(getOverallStatus())}
          </div>
          {!canEditStructure && canEditItemProgress && (
            <div className="px-4 py-2 rounded-lg font-bold border-2 bg-slate-100 text-slate-600 border-slate-300">
              وضع تعديل محدود
            </div>
          )}
          {!canEditStructure && !canEditItemProgress && (
        ...
        <div className="flex gap-3">
          {canManageTemplates && isEditing && (
            <button onClick={saveAsCountryTemplate} className="px-4 py-2 rounded-lg font-bold text-sm bg-purple-600 text-white hover:bg-purple-700">💾 حفظ كقالب</button>
          )}
          {canEditStructure && (
            <button onClick={() => setIsEditing(!isEditing)} className={`px-4 py-2 rounded-lg font-bold ${isEditing ? 'bg-red-500 text-white hover:bg-red-600' : 'bg-slate-800 text-white hover:bg-slate-900'}`}>
      ...
      <div className="flex flex-col xl:flex-row gap-6">
        <div className="xl:w-3/4 space-y-6">
                ...
                <div className="flex justify-between items-center mb-6">
                  <div className="flex items-center gap-3">
                    {allowStructureEditing ? (
                      <input 
                        value={stage.title} 
                        onChange={(e) => updateText(sIdx, null, null, null, e.target.value)} 
                  ...
                  {allowStructureEditing && <button onClick={() => deleteStage(sIdx)} className="text-red-600 font-bold hover:text-red-800">حذف المرحلة</button>}
                </div>
                    ...
                    <div key={step.id} className="bg-white rounded-lg p-5 mb-5 border-2 border-slate-200 shadow-sm hover:shadow-md transition-shadow">
                      ...
                      <div className="flex justify-between items-center mb-4 pb-3 border-b-2 border-slate-200">
                        <div className="flex items-center gap-3">
                          {allowStructureEditing ? (
                            <input 
                              value={step.title} 
                              onChange={(e) => updateText(sIdx, stIdx, null, null, e.target.value)} 
                        ...
                        {allowStructureEditing && <button onClick={() => deleteStep(sIdx, stIdx)} className="text-red-500 text-sm font-bold hover:text-red-700">حذف الخطوة</button>}
                      </div>
                            ...
                            <tr key={item.id} className="border-b border-slate-200 hover:bg-slate-50 transition-colors">
                              <td className="p-3">
                                {allowStructureEditing ? (
                                  <input 
                                    value={item.name} 
                                    onChange={(e) => updateText(sIdx, stIdx, iIdx, 'name', e.target.value)} 
                              ...
                              <td className="p-3">
                                <input 
                                  value={item.note} 
                                  onChange={(e) => updateText(sIdx, stIdx, iIdx, 'note', e.target.value)} 
                              ...
                              <td className="p-3 text-center">
                                <input 
                                  ...
                                  onChange={() => toggleCheckbox(sIdx, stIdx, iIdx)}
                              ...
                              {allowStructureEditing && (
                                <td className="p-3 text-center">
                                  <button 
                                    onClick={() => deleteItem(sIdx, stIdx, iIdx)} 
                      ...
                      {allowStructureEditing && (
                        <div className="flex gap-2">
                          <button 
                            onClick={() => addItem(sIdx, stIdx)} 
                            className="text-blue-600 text-sm font-bold hover:text-blue-800 bg-blue-50 hover:bg-blue-100 px-3 py-1 rounded"
                          >
                            + إضافة بند
                          </button>
                        </div>
          ...
          {allowStructureEditing && (
            <button 
              onClick={addStage} 
              className="w-full bg-slate-800 hover:bg-slate-900 text-white p-4 rounded-xl font-bold transition-all"
            >
              + إضافة مرحلة كبرى
            </button>
          )}
